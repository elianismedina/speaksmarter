...
<template>
    <app-layout title="Lessons">
        <div class="p-6 bg-white rounded-lg shadow">
            <h1 class="text-2xl font-semibold text-gray-800">Lessons</h1>
            <p class="mt-2 text-sm text-gray-600">List of all lessons</p>
        </div>
        <div class="py-4">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="p-6 bg-white border-b border-gray-200 rounded-md shadow-md"
                    v-if="user.permissions.includes('create-lessons')">
                ...
                <div class="overflow-hidden rounded-lg border border-gray-200 shadow-md mt-4"
                    v-if="user.permissions.includes('read-lessons')">
                    <table class="w-full border-collapse bg-white text-left text-sm text-slate-700">
                        <thead class="bg-indigo-500">
                            <tr>
                                <th scope="col" class="px-6 py-4 font-bold text-lg text-slate-100">List of lessons
                                </th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 border-t border-gray-100">
                            <tr class="hover:bg-gray-50" v-for="lesson in lessons.data">
                                <td class="flex flex-col px-6 py-4 h-14">
                                    <div class="">
                                        <p class="text-md font-semibold">{{ lesson.name }}</p>
                                    </div>
                                    <div class="flex flex-row-reverse gap-4 -mt-12 p-2">
                                        <div class="">
                                            <Link @click="deleteLesson(lesson.id)"
                                                v-if="user.permissions.includes('delete-lessons')"
                                        ...
                                        <div>
                                            <Link :href="route('lessons.edit', lesson.id)"
                                                v-if="user.permissions.includes('update-lessons')"
